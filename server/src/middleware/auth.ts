...
export const authenticate = (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        ...
        const cookieToken = rawCookie.split(';').map(c => c.trim()).find(c => c.startsWith('token='))?.split('=')[1];
        const token = cookieToken || req.headers.authorization?.substring(7);
...
export const authorizeCollegeAccess = (req: AuthRequest, res: Response, next: NextFunction) => {
    if (!req.user) {
        return res.status(401).json({ error: 'Not authenticated' });
    }
...
export const checkCollegeLock = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        ...
        return res.status(500).json({ error: 'Failed to check college lock status' });
    }
};

// Middleware for strict role hierarchy enforcement
export const enforceHierarchy = (req: AuthRequest, res: Response, next: NextFunction) => {
    if (!req.user) {
        return res.status(401).json({ error: 'Not authenticated' });
    }
    ...
    if (targetRole && !roleHierarchy[role as keyof typeof roleHierarchy]?.includes(targetRole)) {
        return res.status(403).json({ error: 'Cannot create users with higher or equal privileges' });
    }
